...
/*
 * @param {number|String} id - the id for a tile
 * @return {String} the name of the tile with the given ID
 */
export function getTileName(id) {
  assert(_.has(tileNames, id), `Unknown tile ID: ${id}`);
  const name = _.findKey(tileInfo, tile => tile.id === id);
  assert(name, `Could not find tile name for given tile ID: ${id}`);
  return name;
}
 ...
 */
export function tileHasProperty(name, property) {
  assert(_.has(tileInfo, name), `Unknown tile name: ${name}`);
  return _.has(tileInfo[name], property);
}


/*
 * @param {String} name - the name of the tile whose property we want
 * @param {String} property - the name of a property stored in tileInfo
 * @return the property for the input tile
 */
export function getTileProperty(name, property) {
  assert(_.has(tileInfo, name), `Unknown tile name: ${name}`);
  assert(tileHasProperty(name, property), `Tile does not have property: ${name}, ${property}`);
