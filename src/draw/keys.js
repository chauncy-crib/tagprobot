...
export function drawBlankKeyPresses() {
  ...
  keyPressesVis.addChildAt(
    getPixiSquare(-(1.5 * KEY_SIZE) - KEY_GAP, 0, KEY_SIZE, KEY_OFF_ALPHA, KEY_COLOR),
    KEY_INDICES.left,
  );
  keyPressesVis.addChildAt(
    getPixiSquare(-(0.5 * KEY_SIZE), 0, KEY_SIZE, KEY_OFF_ALPHA, KEY_COLOR),
    KEY_INDICES.down,
  );
  keyPressesVis.addChildAt(
    getPixiSquare((0.5 * KEY_SIZE) + KEY_GAP, 0, KEY_SIZE, KEY_OFF_ALPHA, KEY_COLOR),
    KEY_INDICES.right,
  );
  keyPressesVis.addChildAt(
    getPixiSquare(-(0.5 * KEY_SIZE), -KEY_SIZE - KEY_GAP, KEY_SIZE, KEY_OFF_ALPHA, KEY_COLOR),
    KEY_INDICES.up,
  );
...
function updateKeyPressesDrawing(keyIndex, newAlpha) {
  ...
  keyPressesVis.addChildAt(
    getPixiSquare(xp, yp, KEY_SIZE, newAlpha, KEY_COLOR),
    keyIndex,
  );
}
