...
export function getTileTraversabilityInCells(tileId) {
  ...
  // tile is not fully traversable

  const midCell = (CPTL - 1.0) / 2.0;
  for (let i = 0; i < CPTL; i++) {
    for (let j = 0; j < CPTL; j++) {
      if (tileHasProperty(tileId, 'radius')) { // if tile is circular non-traversable object
      ...
      } else { // tile is noncircular and partially nontraversable (angled walls)
