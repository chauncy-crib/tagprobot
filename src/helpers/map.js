...
export function getMapTraversabilityInCells(map) {
  ...
  for (let i = 0; i < tilesToUpdate.length; i++) {
    ...
    // ensures that when visual mode is turned on, new sprites are generated for all temp-NT tiles.
    // If visual mode is off, then the call to updateNTSprites does nothing.
    if (!areNTSpritesOnScreen()) updateNTSprites(xy.xt, xy.yt, mapTraversabilityCells);
  }
  if (isVisualMode()) setNTSpritesOnScreen(true);
