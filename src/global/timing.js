 ...
 */
const timings = {};
const runningAverageLength = 180; // number of samples to keep track of when calculating timing
...
function startTiming(processName) {
  _.defaults(timings, { [processName]: {} });
...
function stopTiming(processName) {
  assert(
    _.has(timings, processName) && _.has(timings[processName], 'start'),
